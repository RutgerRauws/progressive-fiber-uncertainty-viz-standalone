/*=========================================================================
  ...
  Module:    GaussianFiberSplatter.cxx
...
vtkStandardNewMacro(GaussianFiberSplatter);

//------------------------------------------------------------------------------
// Algorithm and integration into vtkSMPTools
class GaussianFiberSplatterAlgorithm
{
public:
    GaussianFiberSplatter* Splatter;
    double* Scalars;
    ...
    {
    public:
        GaussianFiberSplatterAlgorithm* Algo;
        vtkIdType XMin, XMax, YMin, YMax, ZMin, ZMax;
        vtkIdType ptId;
        Splat(GaussianFiberSplatterAlgorithm* algo) { this->Algo = algo; }
        ...
        void SetPointId(vtkIdType ptId)
        {
            this->ptId = ptId;
        }
                ...
                for (j = this->YMin; j <= this->YMax; j++)
                {
                    ...
                    for (i = this->XMin; i <= this->XMax; i++)
                    {
                        ...
                        if (dist2 <= this->Algo->Radius2)
                        {
                            ...
                            this->Algo->Splatter->AddFibers(
                                    idx,
                                    this->Algo->Scalars + idx,
                                    (*this->Algo->ExistingPointsFibers)[ptId]
                            );
...
GaussianFiberSplatter::GaussianFiberSplatter()
{
...
//------------------------------------------------------------------------------
int GaussianFiberSplatter::RequestInformation(vtkInformation* vtkNotUsed(request),
...
//------------------------------------------------------------------------------
int GaussianFiberSplatter::RequestData(vtkInformation* vtkNotUsed(request),
                                     vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
    ...
    if (totalNumPts == 0)
    {
        //TODO: This is expected behaviour, so we can maybe remove these macros
    ...
    vtkSmartPointer<vtkCompositeDataIterator> dataItr =
            vtkSmartPointer<vtkCompositeDataIterator>::Take(inputComposite->NewIterator());
    ...
    //  its size slightly to accommodate the radius of influence.
    //
    ...
    this->NewPointsFibers = new std::vector<std::vector<std::reference_wrapper<const Fiber>>>(numNewPts);
    ...
    {
        this->Sample = &GaussianFiberSplatter::EccentricGaussian;
    }
    else
    {
        this->Sample = &GaussianFiberSplatter::Gaussian;
    }

    if (this->ScalarWarping && inScalars != nullptr)
    {
        this->SampleFactor = &GaussianFiberSplatter::ScalarSampling;
    }
    else
    {
        this->SampleFactor = &GaussianFiberSplatter::PositionSampling;
    ...
    algo.ExistingPointsFibers = this->ExistingPointsFibers;

    GaussianFiberSplatterAlgorithm::Splat splat(&algo);
    ...
    //
    delete[] this->Visited;
    delete this->NewPointsFibers;

    return 1;
}

//------------------------------------------------------------------------------
void GaussianFiberSplatter::ComputeModelBounds(
...
//        this->ModelBounds[2] >= this->ModelBounds[3] || this->ModelBounds[4] >= this->ModelBounds[5])
    {
        ...
        vtkSmartPointer<vtkCompositeDataIterator> itr =
                vtkSmartPointer<vtkCompositeDataIterator>::Take(input->NewIterator());
...
// input data.
void GaussianFiberSplatter::ComputeModelBounds(
        vtkDataSet* input, vtkImageData* output, vtkInformation* outInfo)
{
    const double* bounds;
    double maxDist;
    int i, adjustBounds = 0;
...
void GaussianFiberSplatter::SetSampleDimensions(int i, int j, int k)
{
    int dim[3];
...
//------------------------------------------------------------------------------
void GaussianFiberSplatter::SetSampleDimensions(int dim[3])
{
    int dataDim, i;
...
//------------------------------------------------------------------------------
void GaussianFiberSplatter::Cap(vtkDoubleArray* s)
{
    int i, j, k;
    vtkIdType idx;
...
//  Gaussian sampling
//
double GaussianFiberSplatter::Gaussian(double cx[3])
{
...
//
double GaussianFiberSplatter::EccentricGaussian(double cx[3])
{
...
//------------------------------------------------------------------------------
const char* GaussianFiberSplatter::GetAccumulationModeAsString()
{
...
//------------------------------------------------------------------------------
void GaussianFiberSplatter::PrintSelf(ostream& os, vtkIndent indent)
{
...
//------------------------------------------------------------------------------
int GaussianFiberSplatter::FillInputPortInformation(int vtkNotUsed(port), vtkInformation* info)
{

...
#ifndef PROGRESSIVE_FIBER_UNCERTAINTY_VIZ_GAUSSIAN_FIBER_SPLATTER_H
#define PROGRESSIVE_FIBER_UNCERTAINTY_VIZ_GAUSSIAN_FIBER_SPLATTER_H
...
class GaussianFiberSplatter : public vtkImageAlgorithm
{

public:
vtkTypeMacro(GaussianFiberSplatter, vtkImageAlgorithm);
     ...
     */
    static GaussianFiberSplatter* New();
     ...
     * properly.
     */
    friend class GaussianFiberSplatterAlgorithm;
    double SamplePoint(double x[3]) // for compilers who can't handle this
    {
        return (this->*Sample)(x);
    }

    void AddFibers(vtkIdType targetPtId, double* sPtr, const std::vector<std::reference_wrapper<const Fiber>>& existingPointFibers)
    {
        auto fibersAtTargetPoint = (*this->NewPointsFibers)[targetPtId];

        if(!this->Visited[targetPtId])
        {
            //In case we have not splatted here before, add all fibers to the voxel
            this->Visited[targetPtId] = 1;
            fibersAtTargetPoint.insert(fibersAtTargetPoint.end(), existingPointFibers.begin(), existingPointFibers.end());
        }
        else
        {
            for(const Fiber& fiberAtExistingPt : existingPointFibers)
            {
                bool isPresent = false;

                for(const Fiber& fiberAtTargetPt : fibersAtTargetPoint)
                {
                    if (fiberAtTargetPt.GetId() == fiberAtExistingPt.GetId())
                    {
                        isPresent = true;
                        break;
                    }
                ...
                if(!isPresent)
                {
                    fibersAtTargetPoint.emplace_back(fiberAtExistingPt);
                }
            }
        }

        *sPtr = fibersAtTargetPoint.size();
    }
...
protected:
    GaussianFiberSplatter();
    ~GaussianFiberSplatter() override = default;
...
private:
    double Radius2;
    double (GaussianFiberSplatter::*Sample)(double x[3]);
    double (GaussianFiberSplatter::*SampleFactor)(double s);
    char* Visited;
...
private:
    GaussianFiberSplatter(const GaussianFiberSplatter&) = delete;
    void operator=(const GaussianFiberSplatter&) = delete;
};


#endif //PROGRESSIVE_FIBER_UNCERTAINTY_VIZ_GAUSSIAN_FIBER_SPLATTER_H
